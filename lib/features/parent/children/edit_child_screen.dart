...
class _EditChildScreenState extends ConsumerState<EditChildScreen> {
  ...
  void _initializeForm() {
    _currentImageUrl = widget.child.imageUrl;
    ...
    try {
      ...
      if (_pickedImage != null) {
        imageUrl = await ref
            ...
            .uploadChildImage(widget.child.id, File(_pickedImage!.path));
      }
      ...
      if (success && mounted) {
        _showSnackBar('Child updated successfully');
        context.pop(true);
      } else {
      ...
      if (mounted) {
        _showSnackBar('Failed to update: $error', isError: true);
      }
    } finally {
      if (mounted) {
